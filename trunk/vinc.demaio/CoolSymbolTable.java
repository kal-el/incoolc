...
public class CoolSymbolTable implements CSymbolTable  {
	 ...
	 * table.
	 */
	public void addId(AbstractSymbol s,AbstractSymbol data,int lineno,AbstractSymbol typename,Object value)throws Exception{
		currentScope.addId(s,data,lineno,typename,value);
	...
	public Scope getMethodScope(Scope classScope,AbstractSymbol s){
		Scope result=null;
		ScopeList methods = classScope.getChildList();
		Iterator<Scope> it = methods.iterator();
		while(it.hasNext()){
			Scope sc = it.next();
			if(sc.getMethodName().getString().equals(s.getString()))
				return sc;				
		}
		return result;
	}
	...
	public ObjectInfo lookup(AbstractSymbol classname,AbstractSymbol featname,AbstractSymbol feattype){
			...
			if(result==null){
				...
				if(cls!=null){
					class_c tmp = classtable.lookup(cls.parent);
				if(tmp!=null && !(tmp.name.getString().equals(TreeConstants.No_class.getString())))
					result = lookup(tmp.name,featname,feattype);
				}
			}
			return result;
	}
	...
	private ObjectInfo lookupAttr(AbstractSymbol classname,
				...
				while(e.hasMoreElements()){
					Feature temp = (Feature)e.nextElement();
						if(temp.getNodeType().getString().equals(TreeConstants.attr.getString())){
							if(((attr)temp).name.getString().equals(featname.getString())){
								result = temp;
								break;
							}
						}
		...
		if(result!=null) obj = new ObjectInfo(TreeConstants.attr,result.lineNumber,((attr)result).type_decl,cls.name);
		return obj;
	}
	...
	private void classWalk(class_c cl) throws Exception{
		...
		while(e.hasMoreElements()){
			...
			if(f.getNodeType().getString().equals(TreeConstants.attr.getString())){
				...
				enterScope();
				getCurrentScope().setMethodName(mtd.name);
				methodWalk(mtd);
				exitScope();
			}
		}
	...
	private void methodWalk(method mtd)throws Exception {
		...
		while(e.hasMoreElements()){
			...
			addId(par.name,TreeConstants.formalc,par.lineNumber,par.type_decl,null);
		}
	...
	private void expressionWalk(Expression body)throws Exception {
		...
		if(strtype.equals(TreeConstants.assign.getString())){
			...
			if(lookup(ex.name)==null){
				...
				if(lookup(self.name,ex.name,TreeConstants.attr)==null)
	...
	private void letWalk(let le) throws Exception{
		addId(le.identifier,TreeConstants.objectID,le.lineNumber,le.type_decl,le.init);
		classtable.lookup(le.type_decl);
		//ObjectInfo info = lookup(AbstractTable.idtable.addString("x"));
		//System.out.println("x è di tipo"+info.getTypename().getString());
	...
	public void setObjectInfo(AbstractSymbol s,ObjectInfo obj)throws Exception{
			if(currentScope.lookup(s)!=null) {
				currentScope.setObjectInfo(s,obj);
			}
			else throw new Exception("ERROR: Trying to update symbol "+s.getString()+" not in scope.");
	}
